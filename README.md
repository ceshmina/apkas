...
雛形のまま残っている箇所は、いまはない。内容を足すときの決まりを以下に書く。
...
ディスコグラフィは新しい順に並べ、行ごと linkcore へ新しいタブでつなぐ。
ジャケットは linkcore のページにある 400px 角の画像を `src/assets/music/` に
置き、`<a class="release">` の `<li>` をコピーして増やす。英語のページでは
linkcore の URL に `?lang=en` を付け、題は配信での英語の表記にする。
