...
if [[ -z "$PROFILE" ]]; then
  ...
  echo "  例: $0 apkas-production.admin" >&2
  exit 1
fi
...
# 同じアカウントに日記サイト（apkas-diary）の state バケットが同居する。
# システムごとにバケットを分け、片方の操作がもう片方に及ばないようにする。
BUCKET="apkas-tfstate-${ACCOUNT_ID}"
...
terraform/envs/production/backend.hcl に以下を記述してください。
